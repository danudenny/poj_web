...
class BackupService extends ScheduleService
{
    ...
    public function checkIn(BackupCheckInRequest $request, int $id) {
        try {
            ...
            $checkInTime = Carbon::parse($employeeBackup->backupTime->start_time);
            $minimumCheckInTime = Carbon::parse($employeeBackup->backupTime->start_time)->addMinutes(-$employeeBackup->backupTime->backup->unit->early_buffer);
            ...
            $employeeBackup->check_in_time = $currentTime;
    ...
    public function checkOut(BackupCheckOutRequest $request, int $id) {
        try {
            ...
            $currentTime = Carbon::now();
            $checkOutTime = Carbon::parse($employeeBackup->backupTime->end_time, 'UTC');
            ...
            $employeeBackup->check_out_time = $currentTime;
