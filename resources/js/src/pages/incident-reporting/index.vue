<template>
    <div class="container-fluid">
        <Breadcrumbs main="Incident Reporting"/>
...
export default {
    ...
    async mounted() {
        ...
        this.initializeIncidentTable();
    },
    methods: {
        async getIncidents() {
            this.loading = true;
            await this.$axios.get(`/api/v1/admin/incident`)
                .then(response => {
        ...
        initializeIncidentTable() {
            const table = new Tabulator(this.$refs.incidentReporting, {
                data: this.incidents,
                layout: 'fitColumns',
                columns: [
                    {
        ...
        viewData(id) {
            this.$router.push({name: 'incident_reporting_detail', params: {id}});
        }
    }
}
</script>
