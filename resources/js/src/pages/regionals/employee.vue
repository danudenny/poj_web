  ...
  methods: {
    initializeEmployeesTable() {
      ...
      this.table = new Tabulator(this.$refs.employeesTable, {
        ...
        ajaxURL: '/api/v1/admin/employee?default_operating_unit_id=' + this.id,
        ajaxConfig: {
          headers: {
        ...
        ajaxURLGenerator: (url, config, params) => {
          ...
          return `${url}&page=${params.page}&size=${params.size}&name=${this.filterName}&email=${this.filterEmail}&kanwil=${this.filterKanwil}&area=${this.filterArea}&cabang=${this.filterCabang}&outlet=${this.filterOutlet}&job=${this.filterJob}`
        },
        layout: 'fitDataFill',
        renderHorizontal:"virtual",
        height: '100%',
        frozenColumn:2,
        groupBy: ['operating_unit.name', 'kanwil.name', 'area.name', 'cabang.name', 'outlet.name'],
